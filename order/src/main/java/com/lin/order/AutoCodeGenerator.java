...
public class AutoCodeGenerator {
    ...
    public static void main(String[] args) {
        ...
        DataSourceConfig.Builder datasourceBuilder = new DataSourceConfig.Builder(
                "jdbc:mysql://localhost:3306/cloud_order?useUnicode=true&characterEncoding=utf-8&zeroDateTimeBehavior=convertToNull&useSSL=false&serverTimezone=UTC",
                "root",
                "root"
        );
        ...
        generator.globalConfig(builder -> {
            ...
            builder.author("linSheng")
                    ...
                    .enableSwagger()
                    // 指定输出目录
                    .outputDir(PROJECT_ROOT_PATH + "/order/src/main/java");
        });

        // 4、包配置
        generator.packageConfig(builder -> {
            // 父包名. ===========1.手动修改设置===========
            builder.parent("com.lin.order.dao")
                    // 将要生成的模块名称. ===========2.手动修改设置===========
//                    .moduleName("employee")
                    // 设置生成的 控制层 文件夹名称
                    ...
                    .pathInfo(Collections.singletonMap(OutputFile.xml, PROJECT_ROOT_PATH + "/order/src/main/java/com/lin/order/dao/mapper"));
        });
        ...
        generator.strategyConfig(builder -> {
            ...
            // ===========3.手动修改设置。===========
            builder.addInclude("order_detail","order_info")
                    // 过滤表前缀，生成的类名会去掉这个前缀
//                    .addTablePrefix("order_", "t_", "sk_")
                    // 第一阶段
                    // 是否生成 entity：是
                    .entityBuilder()
                    // 开启lombok
